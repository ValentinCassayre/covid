...
def graph_country(country_data, date_full_list, y1, y2, graph_format):
    ...
    # mat plot lib
        ...
        plt.ylim(1.2*min(y1), 1.2*max(y1))
        ...
        plt.axhline(y=max(y1), xmin=0, xmax=1, color='blue', alpha=0.5, linestyle=':', linewidth=1, label='Total cases ({:.0f})'.format(max(y1)))
        plt.axhline(y=max(y2), xmin=0, xmax=1, color='red', alpha=0.5, linestyle=':', linewidth=1, label='Total deaths ({:.0f})'.format(max(y2)))
        ...
        plt.xlabel('Date')
        plt.ylabel('Cases')
        plt.savefig("{} covid.{}".format(country_data[0][data_dict["CountryExp"]], graph_format), dpi=None, facecolor='w', edgecolor='w',
...
for n in country_list_number:
    ...
    graph_country(c_data, date_full_list, c_c_new_conf_cases, c_c_new_deaths, graph_format)
